...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    else if ((IsSpeciesOneOf(gBattleMons[gBattlerAttacker].species, gMechobraLine) && GetBattlerHoldEffect(gBattlerAttacker, TRUE) == HOLD_EFFECT_NIKITA))
    {
        if (!JumpIfMoveFailed(7, move))
            RecordAbilityBattle(gBattlerAttacker, ABILITY_NO_GUARD);
        return TRUE;
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (defAbility == ABILITY_ABYSSAL && WEATHER_HAS_EFFECT && gBattleWeather & WEATHER_SANDSTORM_ANY)
        calc = (calc * 80) / 100; // 1.2 sand veil loss
...
static void Cmd_healthbarupdate(void)
{
    ...
    if (gBattleStruct->rouletteEffect == 2 && gBattleMoveDamage > 0)
        gBattleMoveDamage = 0;
...
static void Cmd_resultmessage(void)
{
    ...
    {
        if (gBattleCommunication[6] == 3) // Wonder Guard - show the ability pop-up
    ...
    if (gMoveResultFlags & MOVE_RESULT_MISSED && (!(gMoveResultFlags & MOVE_RESULT_DOESNT_AFFECT_FOE) || gBattleCommunication[6] > 2))
    {
        if (gBattleCommunication[6] == 4) //  Levitate - don't show the ability pop-up
            stringId = gMissStringIds[gBattleCommunication[6]];
            gBattleCommunication[MSG_DISPLAY] = 1;
    }
    else
            ...
            if (statsToCheck & 1)
            {
                if (flags & STAT_CHANGE_CANT_PREVENT)
                {
                    if (gBattleMons[gActiveBattler].statStages[currStat] > MIN_STAT_STAGE)
                    {
                        ...
                        && ability != ABILITY_MEGA_HOUNDOOM_ABILITY
...
static void Cmd_switchineffects(void)
{
    ...
    if (!(gSideStatuses[GetBattlerSide(gActiveBattler)] & SIDE_STATUS_POISON_FIELD_DAMAGED)
     && FlagGet(FLAG_BATTLE_POISON_FIELD))
    {
            ...
            && !(IS_BATTLER_OF_TYPE(gActiveBattler, TYPE_STEEL)))
        {
            ...
            gBattleScripting.battler = gActiveBattler;
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_PoisonFieldPoisoned;
            return;
        }
    }
    ...
    else
    {
        ...
        if (AbilityBattleEffects(ABILITYEFFECT_ON_SWITCHIN, gActiveBattler, 0, 0, 0)
            ...
            || AbilityBattleEffects(ABILITYEFFECT_CHARISMA2, 0, 0, 0, 0)
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_RESET_INTIMIDATE_TRACE_BITS:
        ...
        gSpecialStatuses[gActiveBattler].charismadMon = 0;
    ...
    case VARIOUS_SWITCHIN_ABILITIES:
        ...
        AbilityBattleEffects(ABILITYEFFECT_CHARISMA2, gActiveBattler, 0, 0, 0);
    ...
    if (statValue <= -1) // Stat decrease.
    {
                 ...
                 && notProtectAffected != TRUE && JumpIfMoveAffectedByProtect(0))
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
                  ...
                  || GetBattlerAbility(gActiveBattler) == ABILITY_MEGA_HOUNDOOM_ABILITY)
                 && !certain && gCurrentMove != MOVE_CURSE)
        {
            if (flags == STAT_BUFF_ALLOW_PTR)
            {
