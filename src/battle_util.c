...
{
    if (ability == ABILITY_TARGET_ALL && gBattleMoves[moveId].target == MOVE_TARGET_SELECTED)
...
u8 DoFieldEndTurnEffects(void)
{
    ...
    do
    {
        ...
        case ENDTURN_FAIRY_LOCK:
            ...
            gBattleStruct->turnCountersTracker++;
            break;
        case ENDTURN_ROULETTE:
            if (FlagGet(FLAG_BATTLE_ROULETTE))
            {
                effect = Random() % 6;
                gBattleStruct->rouletteEffect = effect + 1;
                gBattleCommunication[MULTISTRING_CHOOSER] = effect;
                BattleScriptExecute(BattleScript_AnnounceRoulette);
                effect++;
            }
            gBattleStruct->turnCountersTracker++;
            break;
...
else if (gBattleStruct->rouletteEffect == 2) \
{ \
    gBattleStruct->turnEffectsTracker++; \
    break; \
}
...
u8 AbilityBattleEffects(u8 caseID, u8 battler, u16 ability, u8 special, u16 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_SPACE_SHIFT:
            ...
            {
                BattleScriptPushCursorAndCallback(BattleScript_SpaceShiftActivates);
                effect++;
            }
            break;
    ...
    case 1:
        if (gBattleMons[battlerId].hp)
        {
            ...
            case HOLD_EFFECT_NIKITA:
                if (gBattleMons[battlerId].hp < gBattleMons[battlerId].maxHP && !moveTurn)
                {
                    gBattleMoveDamage = gBattleMons[battlerId].maxHP / 12;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    BattleScriptExecute(BattleScript_ItemHealHP_End2);
...
static u32 CalcMoveBasePowerAfterModifiers(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 updateFlags)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN && moveType == TYPE_PSYCHIC && IsBattlerGrounded(battlerAtk) && !(gStatuses3[battlerAtk] & STATUS3_SEMI_INVULNERABLE))
        MulModifier(&modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8) ? UQ_4_12(1.3) : UQ_4_12(1.5));
    if (gFieldStatuses & STATUS_FIELD_DARK_TETHER && moveType == TYPE_DARK && IsBattlerGrounded(battlerAtk) && !(gStatuses3[battlerAtk] & STATUS3_SEMI_INVULNERABLE))
        MulModifier(&modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8) ? UQ_4_12(1.3) : UQ_4_12(1.5));
    if (gFieldStatuses & STATUS_FIELD_DARK_TETHER && moveType == TYPE_DRAGON && IsBattlerGrounded(battlerAtk) && !(gStatuses3[battlerAtk] & STATUS3_SEMI_INVULNERABLE))
        MulModifier(&modifier, UQ_4_12(0.5));
...
u32 CalcAttackStat(u16 move, u8 battlerAtk, u8 battlerDef, u8 moveType, bool32 isCrit, bool32 updateFlags)
{
    ...
    {
        if (IS_MOVE_PHYSICAL(move) == (gBattleStruct->rouletteEffect != 6))
        {
    ...
    case ABILITY_SLOW_START:
        ...
        break;
    case ABILITY_SOLAR_POWER:
        if (stat == STAT_SPATK && WEATHER_HAS_EFFECT && gBattleWeather & WEATHER_SUN_ANY)
            MulModifier(&modifier, UQ_4_12(1.5));
        break;
    ...
    case ABILITY_MINUS:
        if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
        {
            ...
            if (stat == STAT_SPATK && (partnerAbility == ABILITY_PLUS || partnerAbility == ABILITY_MINUS))
                MulModifier(&modifier, UQ_4_12(1.5));
        }
        break;
    case ABILITY_FLOWER_GIFT:
        if (gBattleMons[battlerAtk].species == SPECIES_CHERRIM && WEATHER_HAS_EFFECT && (gBattleWeather & WEATHER_SUN_ANY) && stat == STAT_ATK)
            MulModifier(&modifier, UQ_4_12(1.5));
        break;
    case ABILITY_HUSTLE:
        if (stat == STAT_ATK)
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (gBattleMons[BATTLE_PARTNER(battlerAtk)].species == SPECIES_CHERRIM && stat == STAT_ATK)
                MulModifier(&modifier, UQ_4_12(1.5));
            break;
        }
    }
    ...
    case HOLD_EFFECT_DEEP_SEA_TOOTH:
        if (gBattleMons[battlerAtk].species == SPECIES_CLAMPERL && stat == STAT_SPATK)
            MulModifier(&modifier, UQ_4_12(2.0));
        break;
    ...
    // if (ShouldGetStatBadgeBoost(FLAG_BADGE07_GET, battlerAtk) && stat == STAT_SPATK)
    //     MulModifier(&modifier, UQ_4_12(1.1));
...
u16 GetTypeModifier(u8 atkType, u8 defType)
{
    if ((B_FLAG_INVERSE_BATTLE != 0 && FlagGet(B_FLAG_INVERSE_BATTLE))
     || gBattleStruct->rouletteEffect == 4)
    {
